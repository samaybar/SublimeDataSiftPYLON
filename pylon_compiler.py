...
CHECK_DOWNLOAD_THREAD_TIME_MS = 3000


def monitorDownloadThread(downloadThread):
    if downloadThread.is_alive():
        msg = downloadThread.getCurrentMessage(pylon_type)
...
class HttpRequester(threading.Thread):
    ...
    def _get_settings(self):
        ...
        retval = settings.get('pylon_auth')  or retval
        if not retval:
    ...
    def getResponseTextForPresentation(self, respHeaderText, respBodyText, latencyTimeMilisec, downloadTimeMilisec):
        ...
        if DEBUG:
            ...
            return str(localtime) + " - Request Result: \n" + respBodyText
    def getCurrentMessage(self,endpoint_pylon):
        return "PYLON " + endpoint_pylon + " working..." + str(self.totalBytesDownloaded) + " / " + str(self.contentLenght)
...
class ValidateCsdlCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        pylonendpoint =  "POST https://api.datasift.com/v1.3/pylon/validate"
        global pylon_type
        pylon_type = "validate"
